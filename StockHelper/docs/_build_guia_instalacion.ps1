# Genera la Guia de Instalacion de StockHelper en formato Word (.docx) a partir de
# docs\_guia_instalacion.json y las capturas de la carpeta indicada en imageBaseDir.
#
# Uso:  powershell -ExecutionPolicy Bypass -File .\_build_guia_instalacion.ps1
#
# Requiere Microsoft Word instalado (automatizacion COM). No necesita pandoc.
# Mismo patron que _build_manual_usuario.ps1.

$ErrorActionPreference = "Stop"

$here     = $PSScriptRoot
$jsonPath = Join-Path $here "_guia_instalacion.json"
$outPath  = Join-Path $here "Guia_de_Instalacion_StockHelper.docx"
...
try {
    ...
    function Add-Par([string]$text, [int]$style, [switch]$center, [switch]$italic, [switch]$gray, [switch]$red) {
        $sel.Style = $style
        if ($center) { $sel.ParagraphFormat.Alignment = $wdAlignCenter } else { $sel.ParagraphFormat.Alignment = 0 }
        $sel.Font.Italic = if ($italic) { 1 } else { 0 }
        if ($red)      { $sel.Font.Color = $wdColorRed }
        elseif ($gray) { $sel.Font.Color = $wdColorGray }
        else           { $sel.Font.Color = 0 }
        $sel.TypeText($text)
        $sel.TypeParagraph()
        $sel.Font.Italic = 0
        $sel.Font.Color  = 0
    ...
    function Add-Image([string]$fileName) {
        ...
        if (-not (Test-Path $path)) {
            Write-Host "  ADVERTENCIA: falta imagen $fileName" -ForegroundColor Red
    ...
    foreach ($s in $data.sections) {
        ...
        # Hueco visible para una captura que todavia no se saco.
        if ($s.placeholder) {
            Add-Par ("[ PENDIENTE DE CAPTURA ] " + $s.placeholder) $wdStyleNormal -center -italic -red
            $faltantes.Add($s.placeholder)
        }
    ...
    if ($faltantes.Count -gt 0) {
        Write-Host ""
        Write-Host "CAPTURAS PENDIENTES ($($faltantes.Count)):" -ForegroundColor Yellow
